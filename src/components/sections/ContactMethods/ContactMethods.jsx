	...
	return (
		<section className={styles.section}>
			<div className={styles.inner}>
				<MotionDiv duration={0.4} className={styles.header}>
					<h2 className={styles.title}>Choose how you'd like to connect.</h2>
					<p className={styles.subtitle}>Pick the channel that works best for you — we're responsive on all of them.</p>
				</MotionDiv>

				<MotionDiv className={styles.cards} variants={containerVariants}>
					{methods.map(method => {
						...
						return (
							<MotionDiv inherit key={method.id} variants={cardVariants}>
								{isCalendly ? (
									<button type="button" className={styles.card} onClick={openCalendly} aria-label="Open Calendly to book a call">
										{cardContent}
									</button>
								) : (
									<a href={method.href} target="_blank" rel="noopener noreferrer" className={styles.card}>
										{cardContent}
									</a>
								)}
							</MotionDiv>
